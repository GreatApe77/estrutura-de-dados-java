package com.mateusnavarro77.beecrowd.tabelas_hash;
...
public class Main {
    public static void main(String[] args) throws IOException {
        ...
        for (int i = 0; i < N; i++) {
            ...
            int m = Integer.parseInt(linha1[0]);
            ...
            // String contraBarra = "\\";
            for (int j = 0; j < m; j++) {
                ListaDuplamenteEncadeada<Entrada<Integer, Integer>> lista = tabela.tabela[j];
                ...
                if (lista == null) {
                    ...
                    s.append(j).append(" ->");
                    Nodo<Entrada<Integer, Integer>> atual = lista.primeiro;
                    while (atual != null) {
                        s.append(" ");
                        s.append(atual.getElemento().getValor());
                        s.append(" ");
                        s.append("->");
...
class TabelaHash<K extends Comparable<K>, V> implements Dicionario<K, V> {
    ...
    private int tamanho;
    public ListaDuplamenteEncadeada<Entrada<K, V>>[] tabela;
